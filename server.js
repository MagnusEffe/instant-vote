...
function jsonbinRequest(method, body) {
  return new Promise((resolve, reject) => {
    ...
    // GET : ajouter ?meta=false pour avoir uniquement le record sans métadonnées
    const binPath = method === 'GET'
      ? `/v3/b/${JSONBIN_ID}/latest?meta=false`
      : `/v3/b/${JSONBIN_ID}`;
    const options = {
    ...
    const req = https.request(options, res => {
      ...
      res.on('end', () => {
        ...
        if (res.statusCode < 200 || res.statusCode >= 300) {
          reject(new Error(`JSONBin HTTP ${res.statusCode}: ${data}`));
          return;
        }
...
async function loadState() {
  ...
  try {
    ...
    console.log('JSONBin raw response:', JSON.stringify(res).slice(0, 200));
    // Avec ?meta=false, JSONBin renvoie directement le record (pas enveloppé)
    if (res && res.rooms) {
      state = res;
    } else if (res && res.record && res.record.rooms) {
      // Fallback : format avec métadonnées
      state = res.record;
    }
    if (!state.rooms) state.rooms = {};
